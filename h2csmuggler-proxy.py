...
from h2.events import (ResponseReceived, DataReceived, StreamReset, StreamEnded)
import argparse
...
class MyProxy(http.server.SimpleHTTPRequestHandler):
	...
	def do_GET(self):
		self.init_args.url = self.init_args.url + self.path
		main(self.init_args)
		self.server_version = 'h2csmuggler-proxy'
		self.sys_version = 'v1'
		for name, value in proxy_response_headers:
			if name.decode('utf-8') == ':status':
				self.send_response(int(value.decode('utf-8')))
			elif name.decode('utf-8') == 'date':
				self.send_header('orig_date', value.decode('utf-8'))
			else:
				self.send_header(name.decode('utf-8'), value.decode('utf-8'))
		self.end_headers()
		if 'proxy_response_body' in globals():
			self.wfile.write(proxy_response_body)
			self.wfile.flush()

def handle_events(events, isVerbose):
	global proxy_response_body
	for event in events:
		if isinstance(event, ResponseReceived):
			handle_response(event.headers, event.stream_id)
		elif isinstance(event, DataReceived):
			if 'proxy_response_body' not in globals():
				proxy_response_body = event.data
			else:
				proxy_response_body = proxy_response_body + event.data
...
def handle_response(response_headers, stream_id):
	global proxy_response_headers
	proxy_response_headers = response_headers
...
def main(args):
	...
	#handle_events(events, args.verbose) # No need the response for the initial request
